...
import {
  doesNotificationAmountMatchOrder,
  doesNotificationChainMatchOrder,
  fulfillVoucherOrderPayment,
...
function getNotificationAddress(notification: CircleNotification) {
  const candidateKeys = [
    "walletAddress",
    "fromAddress",
    "sourceAddress",
...
async function updateVoucherOrderStatus(notification: CircleNotification) {
  const mappedStatus = mapCircleStateToStatus(notification.state);
  if (!mappedStatus) return;

  let ordersQuery = supabaseAdminClient
    .from("orders")
    .select("*")
    .order("created_at", { ascending: true });

  if (notification.txHash) {
    ordersQuery = ordersQuery.eq("tx_hash", notification.txHash);
  } else if (notification.id) {
    ordersQuery = ordersQuery.eq("circle_transaction_id", notification.id);
  } else {
    return;
  }

  const { data: matchingOrders, error: orderLookupError } = await ordersQuery;

  if (orderLookupError) {
    console.error("Voucher order lookup error:", orderLookupError);
    return;
  }
  ...
  const notificationWalletAddress = getNotificationAddress(notification);

  for (const order of matchingOrders) {
    if (!doesNotificationAmountMatchOrder(notification.amounts, order)) {
      console.warn(
        `[voucher-order] Skipping ${order.id}: amount mismatch for tx ${notification.txHash}`
      );
      continue;
    }

    if (!doesNotificationChainMatchOrder(notification.blockchain, order)) {
      console.warn(
        `[voucher-order] Skipping ${order.id}: chain mismatch for tx ${notification.txHash}`
      );
      continue;
    }
    ...
    if (
      ...
      notificationWalletAddress !== order.wallet_address.toLowerCase()
    ) {
      console.warn(
        `[voucher-order] Skipping ${order.id}: wallet mismatch for tx ${notification.txHash}`
      );
      continue;
    }

    const { data: destinationWallet, error: destinationWalletError } =
      await supabaseAdminClient
        .from("admin_wallets")
        .select("id")
        .eq("address", order.destination_address ?? "")
        .maybeSingle();

    if (destinationWalletError || !destinationWallet) {
      console.warn(
        `[voucher-order] Skipping ${order.id}: destination ${order.destination_address} is not a managed admin wallet.`
      );
      continue;
    }
    ...
    if (mappedStatus === "failed") {
      if (order.status !== "failed") {
        const { error: failedUpdateError } = await supabaseAdminClient
          .from("orders")
          .update({
            circle_transaction_id: notification.id ?? order.circle_transaction_id,
            status: "failed",
        ...
        if (failedUpdateError) {
          console.error(
            `[voucher-order] Failed to mark order ${order.id} as failed:`,
            failedUpdateError
          );
        }
      }
    ...
    if (mappedStatus === "confirmed" || mappedStatus === "complete") {
      try {
        const result = order.tx_hash
          ? await verifyOrderPayment({
              orderId: order.id,
              order,
              circleTransactionId: notification.id,
            })
          : await fulfillVoucherOrderPayment({
              order,
              circleTransactionId: notification.id,
            });
        ...
        if (!result.reserved) {
          console.warn(
            `[voucher-order] Order ${order.id} was paid but no voucher inventory was available for automatic fulfillment.`
          );
        } else {
          console.log(
            `[voucher-order] Order ${order.id} fulfilled with reserved inventory ${result.order.voucher_inventory_id}.`
          );
        }
      } catch (fulfillmentError) {
        console.error(
          `[voucher-order] Failed to fulfill order ${order.id}:`,
          fulfillmentError
        );
      }
    }
...
async function updateLegacyCreditTransactionStatus(notification: CircleNotification) {
  const mappedStatus = mapCircleStateToStatus(notification.state);
  if (!mappedStatus || !notification.txHash) return;
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (circleTransactionId) {
      await updateVoucherOrderStatus(notification);
      await updateLegacyCreditTransactionStatus(notification);
